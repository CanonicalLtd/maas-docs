...
With previous versions of MAAS (and curtin), network configuration
...
Ephemeral images now use HTTP below.
...
and Windows via cloud-init. The MAAS CentOS images now use the
...
Our thanks to the cloud-init team for improving the network
...
MAAS 2.3 introduces a new hardware testing framework that
...
Historically, MAAS used tgt to provide images over iSCSI for the
...
This is accomplished by providing cloud-init user data. Cloud-init
...
!!! Note:
    This features makes use of the MAAS snap to configure the rack
...
The machines API endpoint now provide more information on configured
...
- Join us on IRC. We can be found on the maas channel on
  freenode. 
- Subscribe to the maas-devel mailing list, a great place to
  ask questions.
